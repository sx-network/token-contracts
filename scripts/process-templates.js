...
glob(directoryPath, (err, files) => {
  ...
  files.forEach((file) => {
    const childChainIdHex = parseInt(program.childChainId, 10).toString(16).toUpperCase();

    const rootChainIdHex = parseInt(program.rootChainId, 10).toString(16).toUpperCase();
