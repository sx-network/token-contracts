...
contract("MintableERC20Predicate", (accounts) => {
  describe("lockTokens", () => {
    ...
    it("Should be able to receive lockTokens tx", async () => {
      ...
      lockTokensTx = await mintableERC20Predicate.lockTokens(
        depositor,
        depositReceiver,
        dummyMintableERC20.address,
        depositData
      );
    ...
    describe("Correct values should be emitted in LockedMintableERC20 log", () => {
      it("Event should be emitted by correct contract", () => {
        lockedLog.address.should.equal(mintableERC20Predicate.address.toLowerCase());
      });
  ...
  describe("lockTokens called by non manager", () => {
    ...
    it("Should revert with correct reason", async () => {
      await expectRevert(
        mintableERC20Predicate.lockTokens(depositor, depositReceiver, dummyMintableERC20.address, depositData, {
          from: depositor,
        }),
  ...
  describe("exitTokens by Alice then deposit back and exitTokens for second time by Bob", () => {
    ...
    it("Alice should be able to deposit amount back", async () => {
      ...
      const lockTokensTx = await mintableERC20Predicate.lockTokens(
        alice,
        alice,
        dummyMintableERC20.address,
        depositData
  ...
  describe("exitTokens with incorrect burn transaction signature", () => {
    ...
    it("Should revert with correct reason", async () => {
      ...
      await expectRevert(
        mintableERC20Predicate.exitTokens(withdrawer, dummyMintableERC20.address, burnLog),
