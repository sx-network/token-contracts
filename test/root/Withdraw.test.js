...
contract("RootChainManager", async (accounts) => {
  describe("Withdraw ERC20", async () => {
    ...
    it("Should fail: exit with a random data receipt", async () => {
      ...
      // start exit
      await expectRevert(
        contracts.root.rootChainManager.exit(data, { from: depositReceiver }),
    ...
    it("Should fail: exit with a fake amount data in receipt", async () => {
      ...
      // start exit
      await expectRevert(contracts.root.rootChainManager.exit(data, { from: depositReceiver }), "revert");
    });
    ...
    it("Should fail to start exit (changed the block number with different encoding)", async () => {
      ...
      // start exit
      await expectRevert.unspecified(contracts.root.rootChainManager.exit(data, { from: depositReceiver }));
    });
  ...
  describe("Withdraw ERC20 :: non-deposit account", async () => {
    ...
    it("Should fail: exit with a random data receipt", async () => {
      ...
      // start exit
      await expectRevert(
        contracts.root.rootChainManager.exit(data, { from: nonDepositAccount }),
        "RootChainManager: INVALID_PROOF"
      );
    });
    ...
    it("Should fail: exit with a fake amount data in receipt", async () => {
      ...
      // start exit
      await expectRevert(contracts.root.rootChainManager.exit(data, { from: nonDepositAccount }), "revert");
    });
    ...
    it("Should fail to start exit (changed the block number with different encoding)", async () => {
      ...
      // start exit
      await expectRevert.unspecified(contracts.root.rootChainManager.exit(data, { from: nonDepositAccount }));
    });
  ...
  describe("Withdraw ERC721", async () => {
    ...
    it("Deposit amount should be credited to correct contract", async () => {
      ...
      newContractBalance.should.be.a.bignumber.that.equals(contractBalance.add(depositAmount));
  ...
  describe("Withdraw batch ERC721", async () => {
    ...
    it("User should be able to exit", async () => {
      const logIndex = withdrawTx.receipt.rawLogs.findIndex(
  ...
  describe("Withdraw single ERC1155", async () => {
    ...
    const depositData = abi.encode(
      ...
      [[tokenId.toString()], [depositAmount.toString()], ["0x0"]]
    );
    let contracts;
    let dummyERC1155;
    let rootChainManager;
    ...
    it("Can receive withdraw tx", async () => {
      withdrawTx = await contracts.child.dummyERC1155.withdrawSingle(tokenId, withdrawAmount, {
  ...
  describe("Withdraw batch ERC1155", async () => {
    ...
    it("Deposit amount should be deducted from depositor account", async () => {
      ...
      newAccountBalanceA.should.be.a.bignumber.that.equals(accountBalanceA.sub(depositAmountA));
      const newAccountBalanceB = await dummyERC1155.balanceOf(accounts[0], tokenIdB);
      newAccountBalanceB.should.be.a.bignumber.that.equals(accountBalanceB.sub(depositAmountB));
      const newAccountBalanceC = await dummyERC1155.balanceOf(accounts[0], tokenIdC);
      newAccountBalanceC.should.be.a.bignumber.that.equals(accountBalanceC.sub(depositAmountC));
      // update account balance
    ...
    it("Deposit amount should be credited to correct contract", async () => {
      ...
      newContractBalanceA.should.be.a.bignumber.that.equals(contractBalanceA.add(depositAmountA));
      const newContractBalanceB = await dummyERC1155.balanceOf(erc1155PredicateAddress, tokenIdB);
      newContractBalanceB.should.be.a.bignumber.that.equals(contractBalanceB.add(depositAmountB));
      const newContractBalanceC = await dummyERC1155.balanceOf(erc1155PredicateAddress, tokenIdC);
      newContractBalanceC.should.be.a.bignumber.that.equals(contractBalanceC.add(depositAmountC));
      // update balance
    ...
    it("Should have less amount in predicate contract after withdraw", async () => {
      const newContractBalanceA = await dummyERC1155.balanceOf(erc1155PredicateAddress, tokenIdA);
      newContractBalanceA.should.be.a.bignumber.that.equals(contractBalanceA.sub(withdrawAmountA));
      const newContractBalanceB = await dummyERC1155.balanceOf(erc1155PredicateAddress, tokenIdB);
      newContractBalanceB.should.be.a.bignumber.that.equals(contractBalanceB.sub(withdrawAmountB));
      const newContractBalanceC = await dummyERC1155.balanceOf(erc1155PredicateAddress, tokenIdC);
      newContractBalanceC.should.be.a.bignumber.that.equals(contractBalanceC.sub(withdrawAmountC));
    });
  });
  ...
  describe("Withdraw MintableERC721", () => {
    ...
    it("Alice should be able to send exit tx", async () => {
      const logIndex = burnTx1.receipt.rawLogs.findIndex(
    ...
    it("Alice should be able to deposit token", async () => {
      ...
      const depositTx = await rootChainManager.depositFor(alice, rootMintableERC721.address, depositData, {
        from: alice,
      });
      should.exist(depositTx);
      const syncTx = await syncState({ tx: depositTx });
    ...
    it("Charlie should be able to deposit token for Daniel", async () => {
      ...
      const depositData = abi.encode(["uint256"], [tokenId.toString()]);
      const depositTx = await rootChainManager.depositFor(daniel, rootMintableERC721.address, depositData, {
        from: charlie,
      });
      should.exist(depositTx);
      const syncTx = await syncState({ tx: depositTx });
